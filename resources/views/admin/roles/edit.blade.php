            ...
            <div class="row mt-4">
                <div class="col">
                    <div class="form-group row">
                        ...
                        <div class="col-md-10">
                            <input type="text" id="name" class="form-control {{ $errors->has('name') ? 'is-invalid' : ''}}" name="name" value="{{ old('name', $role->name) }}" required autocomplete="name" autofocus  maxlength="191" placeholder="{{ __('validation.attributes.backend.access.roles.name') }}">
                            @if ($errors->has('name'))
                                ...
                                @foreach($permissions as $permission)
                                    <div class="checkbox d-flex align-items-center">
                                        <div class="form-check">
                                            <input type="checkbox" name="permissions[]" {{ in_array($permission->name, $rolePermissions) ? 'checked' : '' }} value="{{ $permission->name }}" class="form-check-input switch-input" id="permission-{{ $permission->id }}">
